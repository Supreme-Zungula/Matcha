...
class search_for extends connect {
    ...
    function get_user_bio($username)
    {
        $stmt = "SELECT gender, age, prefereces, biography, fame_rating FROM accounts_bio WHERE UserName=?";
        $sql = $this->conn->prepare($sql);
        $sql->execute([$username]);
        return($sql->fetchAll());
    }

    function search_by_name($name_key)
    {   
        $stmt = "SELECT FirstName, LastName, UserName FROM accounts_basic WHERE UserName=?";
        $sql = $this->conn->prepare($stmt);
        $sql->execute([$name_key]);
        if ($sql->rowCount() > 0)
        {
            $basic = $sql->fetchAll();
            $bio = get_user_bio($name_key);
            $location = get_my_location($name_key);
            $preferences = get_my_prefrance($name_key);
            return array_merge($basic, $bio, $location, $preferences);
        }
        return $sql->fetchAll();
    }
}