<?php
	...
	class User extends connection
	{
			...
			if (isset($_SESSION['email']))
			{
				if (isset($_POST['password']) && isset($_POST['password_confirm']))
				{
					if ($_POST['password'] === $_POST['password_confirm'])
					{
						...
						$query->execute();
						header("Location: http://localhost:8080/matcha/login.php?message=password changed&type=success");	
					}			
					else
					{
						header("Location: http://localhost:8080/matcha/login.php?message=database failure&type=warning");
						die();
					}			
				...
				{
					header("Location: http://localhost:8080/matcha/login.php?message=failure&type=warning");	
				}
			}
			else
			{
				header("Location: http://localhost:8080/matcha/login.php?message=something is wrong&type=danger");
			}
		}
		...
		public function reset_password()
		{
			...
			if (!$result)
			{
				header("Location: http://localhost:8080/matcha/forgot_password.php?message=go and register peasant&type=warning");
			}
			else
			{ 
				session_start();
				$_SESSION['email'] = $_POST['email'];
				$ms = "Click here to reset your password http://localhost:8080/matcha/reset.php>";
				...
				header("Location: http://localhost:8080/matcha/forgot_password.php?message=click the link in your email to reset&type=info");
			}
		}
		...
		public function insert_user()
		{
			if (isset($_POST['register']))
			{
				...
				$ms = "http://localhost:8080/matcha/run.php?verify=true&ash=".$this->confirm."&email=".$email;
		...
		public function sign_user_in()
		{
			...
			{
				session_start();
				header("Location: http://localhost:8080/matcha/home.php");
				return;
			}
			if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['login']))
			{
				...
				try{
					...
					else
					{
						...
						header("Location: http://localhost:8080/matcha/login.php?type=danger&message=Incorrect username/password combination, try again");
					}
		...
		public function verify_user($verification, $hash, $email)
		{
			if ($verification === "true" && isset($hash) && filter_var($email, FILTER_VALIDATE_EMAIL))
			{
				...
				if (!$result)
				{
					header("Location: http://localhost:8080/matcha/login.php?type=danger&message=go fuck yourself peasant");
				...
				else
				{
					...
					header("Location: http://localhost:8080/matcha/login.php?type=success&message=account confirmed, you may now log in");
				}
		...
		public function sign_user_out()
		{
			if ($this->user_is_logged_in() && isset($_GET['log_out']))
			{
				...
				session_destroy();
				header("Location: http://localhost:8080/matcha/",true,303);
				exit;
			}
			return (false);
		}
