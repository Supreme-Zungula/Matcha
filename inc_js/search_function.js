...
{
    var search_key = document.getElementById('search-item');
    var request = mkObj();
    if (request)
    {
        request.onreadystatechange = function()
        {
            ...
            if (request.readyState == 4 && request.status == 200){
                ...
                max_profiles = results.length;
                if (max_profiles == 0)
                    no_results();
                make_profile_div_boxes(max_profiles);
                add_profile_data(results[0].UserName, results[0].profile_picture,
                    results[0].biography, results[0].age, results[0].gender, results[0].preferences,
                    results[0].fame_rating, 0, "not specified");
            }
            console.log('Sending');
            request.open("GET", "../search_page_files/set_search_params.php", true);
            request.setRequestHeader("Content-Type", "application-x-www-urlencoded");
            request.send("search_item=" + search_key.value);
        }
    }
} */
